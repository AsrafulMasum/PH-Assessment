...
function RecipeDetails() {
  const { id } = useParams();
  const { data: recipe } = useLoadSecureData(`/recipes/${id}`);

  return (
    <div className="max-w-screen-xl mx-4 xl:mx-auto mt-28">
      <div className="flex flex-col md:flex-row justify-center gap-20">
        <img className="md:w-1/2 rounded-lg object-cover" src={recipe?.recipeImage} alt="" />
        <div className="md:w-1/2 space-y-2">
          ...
          <p className="text-white">Purchased_by: {recipe?.purchased_by?.map((buyerEmail, idx) => <span key={idx} className="mr-4">{buyerEmail}</span>)}</p>
